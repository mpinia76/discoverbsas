...
while($rs = mysqli_fetch_array($rsTemp)){
	...
	if(($rs['estado']!='2')&&($rs['estado']!='3')){
		...
		$insert = "INSERT INTO reserva_factura_procesada (reserva_id,fecha,cliente,dni,total,neto,diferencia) VALUES (".$rs['id'].",'".date('Y-m-d H:i:s')."','".trim($rs['nombre_apellido'])."','".trim($rs['dni'])."','".$total."','".$neto."','".$diferencia."')";
