...
<?php }elseif(is_array($data) and count($data)>1 and ($_GET['action'] == 'abonar') or ($_POST['datos']) ){
		...
		<fieldset>
			...
			<ul class="form">
				...
				<?php  if($_GET['action'] == 'editar' and $subestado < 3 and ACCION_40){ ?>
					<li><label>Monto neto:</label><input type="text" name="monto" id="monto" value="<?php echo $rs['monto']?>" /></li>
				<?php  }else{ ?>
	...
	<?php  if($rs['nro_orden']==0 and $rs['estado']==0 and $_GET['action'] == 'autorizar'){ ?>
		...
		<p align="center"><input type="button" value="Aprobar" name="aprobar" id="aprobarSubmit" /> <input type="button" value="Desaprobar" name="desaprobar" id="desaprobarSubmit" /></p>
	</form>
