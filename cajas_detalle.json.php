...
$sql = "SELECT rel_pago_operacion.operacion_id, rel_pago_operacion.operacion_tipo, caja_movimiento.id, caja_movimiento.caja_id, caja_movimiento.monto, caja_movimiento.fecha, caja_movimiento.origen, caja_movimiento.registro_id, CONCAT(usuario.apellido,', ',usuario.nombre) as user, caja_movimiento.moneda_id, caja_movimiento.monto_moneda, caja_movimiento.cambio FROM rel_pago_operacion RIGHT JOIN caja_movimiento ON rel_pago_operacion.forma_pago = 'efectivo' AND rel_pago_operacion.forma_pago_id = caja_movimiento.registro_id LEFT JOIN usuario ON caja_movimiento.usuario_id = usuario.id WHERE caja_id =".$_GET['caja_id'];
...
while($rs = mysqli_fetch_array($rsTemp)){
	...
	if($rs['operacion_tipo'] == ''){
		...
		$detalle = $rs['origen'];
									...
									}elseif($es_caja[0] == 'cajaOtra'){
											$detalle = "Transferencia Intersucursal desde ".$es_caja[1]."/Sucursal Ushuaia";
                                    		if ($rs['moneda_id']==2) {
                                            	$detalle .= '(U$S, TC: '.$rs['cambio'].')';
                                            }
                                    		if ($rs['moneda_id']==3) {
                                    ...
                                    }elseif($es_caja[0] == 'haciaOtra'){
                                            $detalle = "Transferencia Intersucursal hacia ".$es_caja[1]."/Sucursal Ushuaia";
                                    		if ($rs['moneda_id']==2) {
                                            	$detalle .= '(U$S, TC: '.$rs['cambio'].')';
                                            }
                                    		if ($rs['moneda_id']==3) {
