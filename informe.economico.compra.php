...
$sql 		= "SELECT $sql_meses,ROUND(SUM(IF(YEAR($tabla.fecha)=$ano,$tabla.monto,0)),2) as 'anual',ROUND(sum($tabla.monto),2) as total, rubro.rubro as tipo, rubro.id as rubro_id FROM $tabla INNER JOIN rubro ON $tabla.rubro_id = rubro.id WHERE rubro.impuestos=1 GROUP BY $tabla.rubro_id";
