...
if(isset($_POST['monto'])){
	...
	$sql = "INSERT INTO compra (nro_orden,fecha,fecha_vencimiento,rubro_id,subrubro_id,proveedor,descripcion,remito_nro,recibo_nro,factura_nro,factura_tipo,factura_orden,monto,user_id,estado)
			VALUES ($nro_orden,'".fechasql($_POST['fecha'])."','".fechasql($_POST['fecha_vencimiento'])."',".$_POST['rubro'].",'".$_POST['subrubro_id']."','".$_POST['proveedor']."','".$_POST['descripcion']."','".$_POST['remito_nro']."','".$_POST['recibo_nro']."','".$_POST['factura_nro']."','".$_POST['factura_tipo']."','".$_POST['factura_orden']."','".$_POST['monto']."',".$_SESSION['useridbsas'].",$estado)";
	mysqli_query($conn,$sql);
	$result = mysqli_error($conn);

	if($result == ''){ $result = 1; }

	$operacion_id 	= mysqli_insert_id($conn);
...
function valida(F) {
	...
	if((F.remito_nro.value!="")&&(F.remito_nro.value.length < 4)) {
		alert("Ingrese al menos 4 digitos en el nro de remito")
		F.factura_nro.focus();
		return false
		}
	if((F.recibo_nro.value!="")&&(F.recibo_nro.value.length < 4)) {
		alert("Ingrese al menos 4 digitos en el nro de recibo")
		F.factura_nro.focus();
		return false
		}
	if((F.factura_tipo.value!="n")&&(F.factura_nro.value.length < 4)) {
...
<div class="formContainer">
	...
	<p align="center"><input type="submit" value="Agregar" name="agregar" /></p>
	</form>
</div>
</body>
</html>
