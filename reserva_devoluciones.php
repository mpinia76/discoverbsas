...
if(isset($_POST['guardar'])){
	...
	if($monto){
		...
		$rs = mysqli_fetch_array(mysqli_query($conn,$sql)); echo mysqli_error($conn);
		...
		if($monto > $cobrado){
		   ...
		   $sql = sprintf("INSERT INTO reserva_devoluciones (reserva_id,usuario_id,forma_pago,fecha,monto,motivo) VALUES (%d,%d,'%s','%s',%d,'%s')",
				...
				mysqli_query($conn,$sql); //echo mysqli_error($conn);
...
<body>
<?php
$sql = "SELECT reservas.*, clientes.*, unidads.* FROM reservas INNER JOIN clientes ON reservas.cliente_id = clientes.id INNER JOIN unidads ON reservas.unidad_id = unidads.id WHERE reservas.id = ".$_GET['reserva_id'];
$reserva = mysqli_fetch_array(mysqli_query($conn,$sql));
?>
<div class="info">
    <strong>Titular: </strong> <?php echo $reserva['nombre_apellido']?> <br/>
    <strong>Unidad: </strong> <?php echo $reserva['unidad']?> <br/>
    <strong>Retiro: </strong> <?php echo fechavista($reserva['retiro'])?> <?php echo $reserva['hora_retiro']?><strong>Devolucion: </strong> <?php echo fechavista($reserva['devolucion']);?> <?php echo $reserva['hora_devolucion']?> hs.
</div>
