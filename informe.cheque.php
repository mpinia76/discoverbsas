...
$sql = "INSERT INTO usuario_log (usuario_id,nombre,accion,ip)
			VALUES ('".$_SESSION['useridbsas']."','".$_SESSION['usernombrebsas']."','Informe de cheques','".getRealIP()."')";
...
while($rs = mysqli_fetch_array($rsTemp)){
	...
	<tr>
		...
		<?php for($i=0;$i<12;$i++){ ?>
		<td class="cheque_librado_fecha_<?php echo $i?>"><?php echo $rs[$i+1]?></td>
		<?php } ?>
...
$sql 		= "SELECT $sql_meses,ROUND(SUM(IF(YEAR($tabla.fecha)=$ano,$tabla.monto,0)),2) as 'anual',ROUND(sum($tabla.monto),2) as total, concat(banco.banco,' ',cuenta.sucursal,' ',cuenta.nombre) as tipo FROM $tabla INNER JOIN cuenta ON $tabla.cuenta_id = cuenta.id AND $tabla.debitado = 1 INNER JOIN cuenta_tipo ON cuenta.cuenta_tipo_id=cuenta_tipo.id INNER JOIN banco ON cuenta.banco_id=banco.id WHERE $tabla.vencido = 0 GROUP BY $tabla.cuenta_id";
...
while($rs = mysqli_fetch_array($rsTemp)){
	...
	<tr>
		<td><?php echo $rs['tipo']?></td>
		<?php for($i=0;$i<12;$i++){ ?>
		<td class="cheque_fecha_<?php echo $i?>"><?php echo $rs[$i+1]?></td>
		<?php } ?>
...
$sql = "SELECT
    ...
    $rs = mysqli_fetch_array(mysqli_query($conn,$sql)); //echo $sql; //echo mysqli_error($conn);
    for($i=0;$i<12;$i++){
?>
		<td width="50" class="pendiente_<?php echo $i?>"><?php echo $rs[$i+1]?></td>
<?php
    }
?>
