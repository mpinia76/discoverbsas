...
$file 	= "cajas_detalle.php"; //archivo
$json	= "cajas_detalle.json.php"; //json
$abm 	= "cajas_detalle.am.php"; //agregar o modificar
...
function doInitGrid(){
	...
	mygrid.setHeader("Fecha, Detalle, Orden/Reserva, Usuario, Credito Euros, Credito USD, Credito $, Debito, Saldo"); 		//nombre de las columnas
	mygrid.attachHeader("#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,&nbsp;");
	mygrid.attachHeader("&nbsp;,&nbsp;,&nbsp;,&nbsp;,${#stat_total},${#stat_total},${#stat_total},${#stat_total},&nbsp;");
	mygrid.enablePaging(true,10,10,"pagingArea",true,"recinfoArea");
    mygrid.setInitWidths("110,*,60,110,80,80,80,80,90"); 				//ancho de las columnas
    mygrid.setColAlign("left,left,right,left,right,right,right,right,right");			//alineacion de las columnas
	mygrid.setColSorting("str,str,int,str,float,float,float,float,float");			//tipo datos para ordenar
	mygrid.setColTypes("ro,ro,ro,ro,price,price,price,price,price");				//editable o no
...
function exportarExcel(){
	...
	mygrid.toExcel('library/grid-excel-php/generate.php?caja=1&nombre=<?php echo $_GET['caja_id']?>&desde='+desde+'&hasta='+hasta);
}
