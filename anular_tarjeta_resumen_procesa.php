...
if(mysqli_num_rows(mysqli_query($conn,$sql)) != 0){
	$rsResumen = mysqli_fetch_array(mysqli_query($conn,$sql));
	if ($rsResumen['estado']==1) {


		$ok=1;
		$sql = "SELECT * FROM rel_pago_operacion WHERE operacion_id = ".$rsResumen['id']." AND operacion_tipo = 'tarjeta_resumen' and forma_pago = 'tarjeta'";
		...
		if(!ACCION_139){
			$sql = "SELECT * FROM rel_pago_operacion WHERE operacion_id = ".$rsResumen['id']." AND operacion_tipo = 'tarjeta_resumen' and forma_pago = 'efectivo'";
		...
		if ($ok) {
			...
			$sql = "DELETE FROM cuenta_a_pagar WHERE operacion_id = ".$rsResumen['id'];
			//echo $sql;
			...
			$sql = "SELECT * FROM rel_pago_operacion WHERE operacion_id = ".$rsResumen['id']." AND operacion_tipo = 'tarjeta_resumen'";
			...
			while($rs = mysqli_fetch_array($rsTemp)){
				...
				$sql = "DELETE FROM rel_pago_operacion WHERE operacion_id = ".$rsResumen['id']." AND operacion_tipo = 'tarjeta_resumen'";

				mysqli_query($conn,$sql);


			}
			$sql = "UPDATE tarjeta_resumen SET
									estado=0
								WHERE id=".$resumen_id;
		...
		echo "El resumen debe estar en estado procesada";
	}
