...
$sql = "SELECT
			tarjeta_consumo_cuota.fecha,CONCAT(tarjeta_consumo_cuota.nro_cuota,'/',tarjeta_consumo.cuotas) AS cuotas,tarjeta_consumo_cuota.monto,'Gasto' as operacion, gasto.nro_orden
		...
		UNION
		SELECT
			tarjeta_consumo_cuota.fecha,CONCAT(tarjeta_consumo_cuota.nro_cuota,'/',tarjeta_consumo.cuotas) AS cuotas,tarjeta_consumo_cuota.monto,'Compra' as operacion, compra.nro_orden
		FROM tarjeta_consumo_cuota
		INNER JOIN tarjeta_consumo
			ON tarjeta_consumo_cuota.tarjeta_consumo_id=tarjeta_consumo.id
		INNER JOIN rel_pago_operacion
...
<form method="post" id="idForm" name="form" action="tarjeta_resumen.pagar.php?resumen_id=<?php echo $resumen_id?>" onSubmit="return valida(this);">
