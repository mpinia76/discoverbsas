...
if(isset($_POST['tarjeta'])){
	...
	if( is_array($operacion_id) and count($operacion_id)>0 ){
		...
		foreach($tarjeta as $key=>$valor){
			...
			//$sql = "SELECT id FROM tarjeta_resumen WHERE estado = 1 AND CONCAT(ano,CONCAT( REPEAT( '0', 2 - LENGTH( mes) ) , mes)) = '".$part[2].intval($part[1])."' AND tarjeta_id = ".$tarjeta_id[$key];
			...
			if(mysqli_fetch_array($rsTempTarjetaResumen)){
				...
				$registro_id = mysqli_insert_id($conn); //numero id en el tipo de pago
