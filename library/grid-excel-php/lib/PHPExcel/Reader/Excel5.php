		...
		for ($i = 0; $i < $countProperties; ++$i) {
			...
			switch ($type) {
				...
				case 0x1E: // null-terminated string prepended by dword string length
					...
					$value = PHPExcel_Shared_phpString::ConvertEncoding($value, 'UTF-8', $codePage);
					$value = rtrim($value);
					break;
		...
		if ( !is_null($this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if (($fmtRuns = $this->_sst[$index]['fmtRuns']) && !$this->_readDataOnly) {
				...
				for ($i = 0; $i <= $sstCount; ++$i) {
					if (isset($fmtRuns[$i])) {
						$text = PHPExcel_Shared_phpString::Substring($this->_sst[$index]['value'], $charPos, $fmtRuns[$i]['charPos'] - $charPos);
						$charPos = $fmtRuns[$i]['charPos'];
					} else {
						$text = PHPExcel_Shared_phpString::Substring($this->_sst[$index]['value'], $charPos, PHPExcel_Shared_phpString::CountCharacters($this->_sst[$index]['value']));
					}

					if (PHPExcel_Shared_phpString::CountCharacters($text) > 0) {
...
//			var_dump($bcBitCount);
	...
	private static function _encodeUTF16($string, $compressed = '')
	{
		...
		return PHPExcel_Shared_phpString::ConvertEncoding($string, 'UTF-8', 'UTF-16LE');
	}
	...
	{
		return PHPExcel_Shared_phpString::ConvertEncoding($string, 'UTF-8', $this->_codepage);
	}
