...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
	...
	*/
	...
	private static function _checkSheetTitle($pValue)
	{
		...
		if (PHPExcel_Shared_phpString::CountCharacters($pValue) > 31) {
	...
	public function setTitle($pValue = 'Worksheet')
	{
		...
		if ($this->getParent()->getSheetByName($pValue)) {
			...
			if (PHPExcel_Shared_phpString::CountCharacters($pValue) > 29) {
				$pValue = PHPExcel_Shared_phpString::Substring($pValue,0,29);
			}
			$i = 1;
			while ($this->getParent()->getSheetByName($pValue . ' ' . $i)) {
				++$i;
				if ($i == 10) {
					if (PHPExcel_Shared_phpString::CountCharacters($pValue) > 28) {
						$pValue = PHPExcel_Shared_phpString::Substring($pValue,0,28);
					}
				} elseif ($i == 100) {
					if (PHPExcel_Shared_phpString::CountCharacters($pValue) > 27) {
						$pValue = PHPExcel_Shared_phpString::Substring($pValue,0,27);
					}
				}
			}
