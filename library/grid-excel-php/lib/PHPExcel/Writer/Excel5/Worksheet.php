...
class PHPExcel_Writer_Excel5_Worksheet extends PHPExcel_Writer_Excel5_BIFFwriter
{
	...
	private function _writeLabelSst($row, $col, $str, $xfIndex)
	{
		...
		$str = PHPExcel_Shared_phpString::UTF8toBIFF8UnicodeLong($str);
	...
	private function _writeStringRecord($stringValue)
	{
		...
		$data = PHPExcel_Shared_phpString::UTF8toBIFF8UnicodeLong($stringValue);
	...
	function _writeUrlInternal($row1, $col1, $row2, $col2, $url)
	{
		...
		// character count
		$url_len = PHPExcel_Shared_phpString::CountCharacters($url);
		$url_len = pack('V', $url_len);

		$url = PHPExcel_Shared_phpString::ConvertEncoding($url, 'UTF-16LE', 'UTF-8');
	...
	private function _writeRangeProtection()
	{
		foreach ($this->_phpSheet->getProtectedCells() as $range => $password) {
			...
			$recordData .= PHPExcel_Shared_phpString::UTF8toBIFF8UnicodeLong('p' . md5($recordData));
	...
	private function _writeHeader()
	{
		...
		*/
		...
		if ($this->_BIFF_version == 0x0600) {
			$recordData = PHPExcel_Shared_phpString::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddHeader());
			$length = strlen($recordData);
		} else {
	...
	private function _writeFooter()
	{
		...
		*/
		...
		if ($this->_BIFF_version == 0x0600) {
			$recordData = PHPExcel_Shared_phpString::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddFooter());
			$length = strlen($recordData);
		} else {
	...
	private function _writeDataValidity()
	{
		...
		$this->_append($header.$data);
			...
			foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
				...
				$data .= PHPExcel_Shared_phpString::UTF8toBIFF8UnicodeLong($promptTitle);

				// error title
				$errorTitle = $dataValidation->getErrorTitle() !== '' ?
					$dataValidation->getErrorTitle() : chr(0);
				$data .= PHPExcel_Shared_phpString::UTF8toBIFF8UnicodeLong($errorTitle);

				// prompt text
				$prompt = $dataValidation->getPrompt() !== '' ?
					$dataValidation->getPrompt() : chr(0);
				$data .= PHPExcel_Shared_phpString::UTF8toBIFF8UnicodeLong($prompt);

				// error text
				$error = $dataValidation->getError() !== '' ?
					$dataValidation->getError() : chr(0);
				$data .= PHPExcel_Shared_phpString::UTF8toBIFF8UnicodeLong($error);
