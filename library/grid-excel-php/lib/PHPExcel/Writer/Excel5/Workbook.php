...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter
{
	...
	function _calcSheetOffsets()
	{
		...
		foreach ($this->_phpExcel->getWorksheetIterator() as $sheet) {
			if ($this->_BIFF_version == 0x0600) {
				$offset += $boundsheet_length + strlen(PHPExcel_Shared_phpString::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
			} else {
	...
	private function _writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
	{
		...
		$nlen = PHPExcel_Shared_phpString::CountCharacters($name);

		// name with stripped length field
		$name = substr(PHPExcel_Shared_phpString::UTF8toBIFF8UnicodeLong($name), 2);
	...
	private function _writeBoundsheet($sheet, $offset)
	{
		...
		if ($this->_BIFF_version == 0x0600) {
			...
			$data .= PHPExcel_Shared_phpString::UTF8toBIFF8UnicodeShort($sheetname);
		} else {
	...
	private function _writeNumFormat($format, $ifmt)
	{
		...
		if ($this->_BIFF_version == 0x0600) {
			$numberFormatString = PHPExcel_Shared_phpString::UTF8toBIFF8UnicodeLong($format);
