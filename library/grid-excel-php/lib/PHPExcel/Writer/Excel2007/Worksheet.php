...
class PHPExcel_Writer_Excel2007_Worksheet extends PHPExcel_Writer_Excel2007_WriterPart
{
	...
	private function _writeSheetFormatPr(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null)
	{
		...
		$objWriter->startElement('sheetFormatPr');
			...
			if ($pSheet->getDefaultRowDimension()->getRowHeight() >= 0) {
				...
				$objWriter->writeAttribute('defaultRowHeight',	PHPExcel_Shared_phpString::FormatNumber($pSheet->getDefaultRowDimension()->getRowHeight()));
			} else {
			...
			if ($pSheet->getDefaultColumnDimension()->getWidth() >= 0) {
				$objWriter->writeAttribute('defaultColWidth', PHPExcel_Shared_phpString::FormatNumber($pSheet->getDefaultColumnDimension()->getWidth()));
			}
			...
			$objWriter->startElement('cols');
				...
				foreach ($pSheet->getColumnDimensions() as $colDimension) {
					...
					if ($colDimension->getWidth() < 0) {
						...
						// Width set
						$objWriter->writeAttribute('width',		PHPExcel_Shared_phpString::FormatNumber($colDimension->getWidth()));
					}
	...
	private function _writePageMargins(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null)
	{
		...
		$objWriter->writeAttribute('left',		PHPExcel_Shared_phpString::FormatNumber($pSheet->getPageMargins()->getLeft()));
		$objWriter->writeAttribute('right',		PHPExcel_Shared_phpString::FormatNumber($pSheet->getPageMargins()->getRight()));
		$objWriter->writeAttribute('top',		PHPExcel_Shared_phpString::FormatNumber($pSheet->getPageMargins()->getTop()));
		$objWriter->writeAttribute('bottom',	PHPExcel_Shared_phpString::FormatNumber($pSheet->getPageMargins()->getBottom()));
		$objWriter->writeAttribute('header',	PHPExcel_Shared_phpString::FormatNumber($pSheet->getPageMargins()->getHeader()));
		$objWriter->writeAttribute('footer',	PHPExcel_Shared_phpString::FormatNumber($pSheet->getPageMargins()->getFooter()));
		$objWriter->endElement();
	}
				...
				while($currentRow++ < $highestRow) {
					...
					if ($writeCurrentRow) {
						...
						if ($rowDimension->getRowHeight() >= 0) {
							...
							$objWriter->writeAttribute('ht',			PHPExcel_Shared_phpString::FormatNumber($rowDimension->getRowHeight()));
						}
				...
				switch (strtolower($mappedType)) {
					case 'inlinestr':	// Inline string
						if (! $cellValue instanceof PHPExcel_RichText) {
							$objWriter->writeElement('t', PHPExcel_Shared_phpString::ControlCharacterPHP2OOXML( htmlspecialchars($cellValue) ) );
						...
						if ($this->getParentWriter()->getOffice2003Compatibility() === false) {
							if ($this->getParentWriter()->getPreCalculateFormulas()) {
								...
								if (!is_array($calculatedValue) && substr($calculatedValue, 0, 1) != '#') {
									$objWriter->writeElement('v', PHPExcel_Shared_phpString::FormatNumber($calculatedValue));
								} else {
