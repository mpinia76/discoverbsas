...
<select size="1" name="metodo" id="metodo">
	<option value="devolucion" <?php if($_POST['metodo'] == 'devolucion'){?> selected="selected" <?php } ?> >Por fecha de devoluci&oacute;n</option>
...
<select size="1" name="puntos" id="puntos">
	...
	<?php
	$sql = "SELECT id, CONCAT(numero,' ', cuit,' ', descripcion,' ', direccion) as punto FROM punto_ventas WHERE ivaVentas = 1 ";
...
<th colspan="6" style="text-align:center; font-size:16px">Pantalla</th>

<th></th>
<th colspan="3" style="text-align:center; font-size:16px">Mes seleccionado</th>
...
<th></th>
<th colspan="3" style="text-align:left; background-color:#15d905; font-size:16px">Facturado: $<span id="montoMesFacturado"></span></th>
...
$sql = "INSERT INTO usuario_log (usuario_id,nombre,accion,ip)
			VALUES ('".$_SESSION['useridbsas']."','".$_SESSION['usernombrebsas']."','Facturacion electronica','".getRealIP()."')";
...
while($rs = mysqli_fetch_array($rsTemp)){
	...
	if(($rs['estado']!='2')&&($rs['estado']!='3')){
		...
		//echo $transferencias.'+'.$tarjetas.'+'.$cheques.' / ';
...
if ($mostrar) {
	if (($estado == 'Pendiente')||($estado == 'Facturacion Parcial')) {
	    //echo $fc.'-'.$facturas.'+'.$otrasFacturas.'/';
...
$('#descargas').change(function(){
    ...
    if($(this).val()!=''){
        ...
        $.ajax({
            ...
            data: datos,
            url: 'functions/damePuntoVentas.php',
            success: function(data) {

                $('#punto_combo').html(data);
