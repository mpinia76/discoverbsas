...
<?php
        ...
        <?php
            $sql = "SELECT DISTINCT empleado_adelanto.id,empleado_adelanto.creado, empleado_adelanto.monto, empleado_adelanto.comentarios, CONCAT(usuario.apellido,', ',usuario.nombre) as user, caja.caja, cuenta.sucursal, cuenta.nombre, banco.banco, ec.fecha, cuenta_movimiento.fecha as fecha_cuenta  FROM empleado_adelanto LEFT JOIN usuario ON empleado_adelanto.creado_por = usuario.id LEFT JOIN rel_pago_operacion rpo ON empleado_adelanto.id = rpo.operacion_id AND rpo.operacion_tipo = 'sueldo_adelanto' LEFT JOIN efectivo_consumo ec ON rpo.forma_pago_id = ec.id AND rpo.forma_pago = 'efectivo' LEFT JOIN caja_movimiento cm ON cm.registro_id = ec.id AND cm.origen = 'efectivo_consumo' LEFT JOIN caja ON cm.caja_id = caja.id LEFT JOIN rel_pago_operacion ON empleado_adelanto.id = rel_pago_operacion.operacion_id AND rel_pago_operacion.operacion_tipo = 'sueldo_adelanto' LEFT JOIN cuenta_movimiento ON rel_pago_operacion.forma_pago_id = cuenta_movimiento.id LEFT JOIN cheque_consumo ON cuenta_movimiento.registro_id = cheque_consumo.id AND cuenta_movimiento.origen = 'cheque' LEFT JOIN cuenta ON cuenta_movimiento.cuenta_id = cuenta.id LEFT JOIN banco ON cuenta.banco_id = banco.id WHERE empleado_adelanto.empleado_id = $empleado_id AND empleado_adelanto.mes = $mes AND empleado_adelanto.ano = $ano";
            //echo $sql;
            ...
            if(mysqli_num_rows($rsTemp) > 0) {
                while($rs = mysqli_fetch_array($rsTemp)){
                	...
                	?>
                <?php
                $caja = ($rs['caja'])? 'Caja: '.$rs['caja'].' ('.fechavista($rs['fecha']).')':'';
        $cuenta = ($rs['sucursal'])? 'Cuenta: '.$rs['banco'].' ('.$rs['sucursal'].') '.$rs['nombre'].' ('.fechavista($rs['fecha_cuenta']).')':'';
