 ...
 ?>
    ...
    <table width="100%" cellspacing="0" id="EditTableEstadia" style="table-layout:fixed;"> 
     <tbody>
     <col width="70px" />
...
function imprimir(){
	...
	if ((desde!='undefined-undefined-')) {
    	document.location = "<?php echo $this->Html->url('/categorias/estadias', true);?>/"+desde+"/"+hasta+"/1";
    	}
...
$this->Js->buffer('
    ...
    position = dhxWins.window("w_estadia_categorias").getPosition();
    xpos = position[0];
    ypos = position[1];
');
?>
$('#EditTableEstadia td').dblclick(function () {
     var id = $(this).attr('id');
     varIdArray = id.split(" "); 
     var strDesde = $('#desde').val().split("/"); 
	 var desde = strDesde[2]+'-'+strDesde[1]+'-'+strDesde[0];	
     ...
     if(id){
     	createWindow("w_categorias_estadias_view","Estadias Minimas","<?php echo $this->Html->url('/categorias/editar_estadia', true);?>/"+varIdArray[0]+"/"+desde+"/"+hasta+"/"+dias,"450","200");
     }
});
