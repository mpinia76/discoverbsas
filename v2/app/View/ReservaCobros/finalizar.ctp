...
<div class="ym-grid">
    <div class="ym-g50 ym-gl">
        <div class="ym-gbox">
            ...
            <strong>Unidad: </strong> <?php echo $reserva['Unidad']['unidad']; ?> <br/>
            <strong>Lugar Retiro: </strong> <?php echo $reserva['Lugar_Retiro']['lugar']; ?> <br/>
            <strong>Retiro: </strong> <?php echo $reserva['Reserva']['retiro'];?><?php echo $reserva['Reserva']['hora_retiro'];?> hs.
        </div>
    </div>
...
<div class="sectionTitle" style="margin-top: 10px;">KM de la Unidad</div>
<div class="ym-grid">
    <div class="ym-g25 ym-gl"><?php echo $this->Form->input('Reserva.km_ini',array('label' => 'KM inicio contrato','value' => $reserva['Reserva']['km_ini'])); ?></div>
    <div class="ym-g25 ym-gl"><?php echo $this->Form->input('Reserva.km_fin',array('label' => 'KM finales','value' => ($reserva['Reserva']['km_fin'])?$reserva['Reserva']['km_fin']:'')); ?></div>
    <div class="ym-g25 ym-gl"><?php echo $this->Form->input('Reserva.km_totales',array('label' => 'KM totales', 'type'=>'number','value' => ($reserva['Reserva']['km_fin'])?$reserva['Reserva']['km_fin']-$reserva['Reserva']['km_ini']:0)); ?></div>
    <div class="ym-g25 ym-gr"><span id="guardarKM" style="margin-top: 15px;" onclick="guardar('<?php echo $this->Html->url('/reservas/guardarKM.json', true);?>',$('form').serialize());"  class="boton guardar"> Guardar </span></div>
</div>
<div style="color:red" id="otras_reservas"><?php echo $otras_reservas_speech;?></div>
...
<div class="ym-gbox">
    <table width="100%">
        ...
        ?>
        <tr>
            <td><?php if($permisoEditar){?><a onclick="editarFactura('<?php echo $factura['ReservaFactura']['id'];?>')">editar</a>&nbsp;&nbsp;&nbsp;<a onclick="eliminarFactura('<?php echo $factura['ReservaFactura']['id'];?>')">eliminar</a>&nbsp;&nbsp;&nbsp;<?php }?></td>
...
$('#ReservaKmFin').change(function(){

    var inicio = 0;
    var fin=0;
    if($('#ReservaKmIni').val()!=''){
        inicio = parseInt($('#ReservaKmIni').val());
    }
    if($('#ReservaKmFin').val()!=''){
        fin = parseInt($('#ReservaKmFin').val());
    }
    var total = fin-inicio;
    if(total<0){
        //alert('KM finales NO puede ser menor que KM iniciales');
    }
    else{
        $('#ReservaKmTotales').val(parseInt(total));
    }
...
function extras_facturas(id){

    createWindow('w_reservas_finalizar_'+id,'Finalizar reserva','<?php echo $this->Html->url('/reserva_cobros/finalizar', true);?>/'+id,'800','500');
    refreshOnClose('w_reservas_finalizar_'+id);
