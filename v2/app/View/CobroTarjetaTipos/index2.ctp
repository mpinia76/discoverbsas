    ...
    oTable = $("#dataTable").dataTable( {
        ...
        "fnDrawCallback": function( oSettings ) {
            $("#dataTable tr").unbind("dblclick").dblclick(function(){
                ...
                createWindow("w_'.$this->params['controller'].'_view","Editar","'.$this->Html->url('/'.$this->params['controller'].'/edit2', true).'/"+data[0],"430","350");
            });
...
function edit(){
    ...
    createWindow('w_<?php echo $this->params['controller'];?>_edit','Editar','<?php echo $this->Html->url('/'.$this->params['controller'].'/edit2', true);?>/'+data,'430','350');
}
</script>
<ul class="action_bar">
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="dataTable">
    <thead>
        <tr>
            <th>id</th>
            <th>Cobro Tarjeta Posnet</th>
            <th>Marca</th>
            <th>Cuenta</th>
            <th>Nro Comercio</th>
            <th>Id DECIDIR</th>
            <th>Disponible venta on line</th>
            <th>Mostrar al cobrar</th>
        </tr>
    </thead>
    <tbody>
