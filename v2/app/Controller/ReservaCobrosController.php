...
class ReservaCobrosController extends AppController {
    ...
    public function finalizar($reserva_id, $restringido=0){
        ...
        $this->Reserva->id = $reserva_id;
        $reserva = $this->Reserva->read();
        if(empty($reserva['Reserva']['km_ini'])){
            $this->loadModel('Unidad');
            $this->Unidad->id = $reserva['Unidad']['id'];
            $unidad = $this->Unidad->read();
            $reserva['Reserva']['km_ini']=$unidad['Unidad']['km'];
        }

        //print_r($reserva['Reserva']);

        $date_parts = explode("/",$reserva['Reserva']['devolucion']);
        $devolucion = $date_parts[2]."-".$date_parts[1]."-".$date_parts[0];
        if ($devolucion <= date("Y-m-d")){
            $this->loadModel('ConfiguracionReserva');
            $confReserva = $this->ConfiguracionReserva->find('first',array('conditions' => array('id =' =>'nro_reserva_inicial')));

            $nro_reserva = 0;
            if ($confReserva) {
                $nro_reserva = $confReserva['ConfiguracionReserva']['valor'];
            }

            $filtroNro=array('Reserva.numero >=' => $nro_reserva);
            $filtroUnidad=array('Reserva.unidad_id' => $reserva['Unidad']['id']);
            ...
            $condicion=array($filtroNro,$filtroUnidad,$filtroDevolucion,$filtroEstado,$filtroKm);

            $reservasAnteriores = $this->Reserva->find('all', array(
                'conditions' => $condicion, 'order' => 'devolucion ASC'
            ));
            ...
            //print_r($reservasAnteriores);
            if(count($reservasAnteriores) > 0){
                $reservasSinKM='';
                foreach($reservasAnteriores as $reservasAnterior){
                    $reservasSinKM .='<a href="#" onClick="extras_facturas('.$reservasAnterior['Reserva']['id'].')">'.$reservasAnterior['Reserva']['numero'].'</a>'.', ';
                }
                $otrasReservas=1;
                $otras_reservas_speech='Existen contratos sin carga de KM finales. Reservas: '.$reservasSinKM;
            }
            else{
                $otrasReservas=0;
                $otras_reservas_speech='';
            }
        }
        else{
            $otrasReservas=1;
        ...
        $this->set('otrasReservas',$otrasReservas);

        $this->set('otras_reservas_speech',$otras_reservas_speech);
        $this->set('reserva',$reserva);

        $facturas = $this->Reserva->ReservaFactura->find('all',array('conditions' => array('reserva_id' => $reserva_id), 'order' => 'fecha_emision asc','recursive' => 2));
    ...
    public function detalle($cobro_id){
        ...
        switch($cobro['ReservaCobro']['tipo']){
            ...
            case 'TRANSFERENCIA':
                ...
                $this->Usuario->id = $_COOKIE['useridbsas'];
