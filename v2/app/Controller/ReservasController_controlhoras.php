...
class ReservasController extends AppController {
	...
	public function get_reservas_check($desde, $hasta) {
 	...
 	public function dataTable2(){
		...
		$rows = array();
 		...
 		$condicion =array('OR' =>array(array('fecha' => $desde),array('fecha' => $hasta)),'abre '=>0);
	...
	'DiaHorario.hora_fin'),'conditions'=>array('SemanaDia.dia = '=>$diaDesde),'recursive' => -1));
        	...
        	foreach ($diaHorarioDesde as $horarios) {
        		//print_r($horarios);
 				...
 				if (($HoraRetiro.':00'>=$horarios['DiaHorario']['hora_inicio'])&&($HoraRetiro.':00'<=$horarios['DiaHorario']['hora_fin'])) {
 			...
 			if ($okHorariosDesde) {
				...
				$diaHorarioHasta = $this->SemanaDia->find('all',array('joins' => array(
		        array(
		            'table' => 'dia_horarios',
		            'alias' => 'DiaHorario',
		            'type' => 'LEFT',
		...
		'DiaHorario.hora_fin'),'conditions'=>array('SemanaDia.dia = '=>$diaHasta),'recursive' => -1));

	 			$okHorariosHasta=1;
	        	foreach ($diaHorarioHasta as $horarios) {
	 				$okHorariosHasta=0;
	 				if (($HoraRetiro.':00'>=$horarios['DiaHorario']['hora_inicio'])&&($HoraRetiro.':00'<=$horarios['DiaHorario']['hora_fin'])) {
	 					$okHorariosHasta=1;
	 				}
	        	...
	        	if ($okHorariosHasta) {
		        	$condicion =array('fecha' => $desde,'abre '=>1);
		 			$feriadosDesde = $this->Feriado->find('first',array('conditions' => $condicion));

		 			$okFeriadosDesde=1;
		        	foreach ($feriadosDesde['FeriadoHorario'] as $horarios) {
		 				$okFeriadosDesde=0;
		 				//echo $HoraRetiro.'>='.$horarios['hora_inicio'].')&&('.$HoraRetiro.'<='.$horarios['hora_fin'];
		 				if (($HoraRetiro.':00'>=$horarios['hora_inicio'])&&($HoraRetiro.':00'<=$horarios['hora_fin'])) {
		 					$okFeriadosDesde=1;
		 				}
		 			...
		 			if ($okFeriadosDesde) {
			 			$condicion =array('fecha' => $hasta,'abre '=>1);
			 			$feriadosHasta = $this->Feriado->find('first',array('conditions' => $condicion));
			 			$okFeriadosHasta=1;
			        	foreach ($feriadosHasta['FeriadoHorario'] as $horarios) {
			 				$okFeriadosHasta=0;
        	...
        	if(array_key_exists('ReservaExtraId',$this->request->data)){
                ...
                if($reservaextras and count($reservaextras)>0){
                    ...
                    foreach($reservaextras as $extra){
                    	$total += $this->request->data['ReservaExtraPrecio'][$i];
	...
	public function formMailPlanilla($reserva_id){
 	...
 	public function enviarPlanilla(){
		...
		$headers  = "From: Discover Ushuaia-rent a car <info@discoverbuenosairesrentacar.com.ar> ".$eol;

	    $headers .= "Return-path: Discover Ushuaia-rent a car <info@discoverbuenosairesrentacar.com.ar> ".$eol;
