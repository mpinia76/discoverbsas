...
class CobroTarjetaCuotasController extends AppController {
	...
	public function dataTable($limit = ""){
        $rows = array();
        $this->loadModel('CobroTarjetaCuota');
        if($limit == "todos"){
            $unidades = $this->CobroTarjetaCuota->find('all'); 
        }else{
            $unidades = $this->CobroTarjetaCuota->find('all',array('limit' => $limit)); 
        }
       
        $this->loadModel('CobroTarjetaTipo');
       $tarjeta_tipos = $this->CobroTarjetaTipo->find('all');
       
       foreach($tarjeta_tipos as $tipo){
            foreach($tipo['CobroTarjetaCuota'] as $cuota){ 
            	$estado = ($cuota['activa'])?'Si':'No';
            	$rows[] = array(
	                $cuota['id'],
	                $tipo['CobroTarjetaPosnet']['posnet'],
	                $tipo['CobroTarjetaTipo']['marca'],
	                $cuota['cuota'],
	                $cuota['mascara_web'],
	                $cuota['interes'],
	                $estado
