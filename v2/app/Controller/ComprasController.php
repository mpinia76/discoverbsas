   ...
   public function index(){
    ...
    public function dataTable(){
    	...
    	switch ($_GET['iSortCol_0']) {
			...
			break;
			case 8:
			$order='Compra.factura_tipo '.$orderType.', Compra.factura_nro '.$orderType;
			break;
			case 9:
        ...
        if ($user['Usuario']['admin'] != '1'){
	    	...
	    	foreach($permisos as $permiso){
	               if ($permiso['UsuarioPermiso']['permiso_id']==128) {
	               		$tienePermiso=1;
	               		continue;
	               }
	        }
    ...
    /* Cache query */
     public function get_compras($offset, $limit, $order, $search, $search1, $search2, $search3, $search4, $search5, $search6, $search7, $search8, $search10, $search11) {
        ...
        if (!$result) {
        	...
        	$condicionSearch8 = ($search8)?array('or' => array('Compra.factura_tipo LIKE '=>'%'.$search8.'%', 'Compra.factura_nro LIKE '=>'%'.$search8.'%', 'Compra.recibo_nro LIKE '=>'%'.$search8.'%')):array();
        	switch ($search10) {
        		case 'Esperando nro. orden':
        		$condicionSearch10 = array('Compra.estado = '=>0, 'Compra.nro_orden = '=>0);
        		break;
