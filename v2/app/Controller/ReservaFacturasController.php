				...
				foreach ($value['ModelName'] as $tit => $val) {
          			...
          			switch (trim($tit)) {
          				...
          				case 'Tipo:':
          					...
          					$pos = strpos(trim($val), 'actura ');
          					if ($pos != false) {
          						$tipoItem=str_replace('Factura ', '',trim($val));
          						$tipoDocItem=1;
          					}
          					$pos = strpos(trim($val), 'ota de Crédito ');
          					if ($pos != false) {
          						$tipoItem=str_replace('Nota de Crédito ', '',trim($val));
          						$tipoDocItem=2;
          					}
          				break;
          				case 'Comp':
					...
					if ($puntoVentaNumero!=$puntoVenta['PuntoVenta']['numero']) {
						...
						if (!$reservaFacturaProcesada) {  
							...
							if ($reservaFactura) {  
								...
								$this->ReservaFactura->set('tipoDoc',$tipoDocItem);
